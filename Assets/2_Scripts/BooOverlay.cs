...
public class BooOverlay : MonoBehaviour
{
    ...
    public float posZDefault, scaleDefault, scaleIndex;
        ...
        if (manager && manager.IsInitialized() && foregroundCamera)
        {
            ...
            if (manager.IsJointTracked(userId, iJointIndex))
            {
                ...
                if (posJoint != Vector3.zero)
                {
                    ...
                    if (overlayObject)
                    {
                        overlayObject.localPosition = posJoint;
                        float scale = scaleDefault - (posJoint.z - posZDefault) * scaleIndex;
                        overlayObject.localScale = new Vector3(scale, scale, scale);
                        Quaternion rotJoint = manager.GetJointOrientation(userId, iJointIndex, !objFlipped);
                        rotJoint = initialRotation * rotJoint;
                        overlayObject.localRotation = Quaternion.Slerp(overlayObject.rotation, rotJoint, smoothFactor * Time.deltaTime);
                    }
                }
            }
            else
            {
                ...
                {
                    Vector3 posJoint = overlayObject.localPosition;
                    posJoint.z = -100f;
                    overlayObject.localPosition = posJoint;
                }
            }
